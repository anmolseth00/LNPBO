...
def _sample_pairs(indices, y, max_pairs, rng):
    ...
    for _ in range(max_attempts):
        ...
        if key in seen:
            continue
        seen.add(key)
        # Fixed convention: (winner, loser, 1) — winner is the higher-y point.
...
def train_bt_model(X, y, group_ids=None, epochs=20, batch_size=1024, seed=42):
    ...
    if len(pairs) == 0:
        raise ValueError(
            "Bradley-Terry requires >=2 distinct objective values to form "
            "preference pairs; got none after within-group sampling."
        )
