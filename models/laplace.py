...
def train_mlp(model, X_train, y_train, epochs=100, lr=1e-3, batch_size=256, weight_decay=0.0):
    ...
    Set ``weight_decay`` to ``prior_precision / N`` so the MAP is consistent
    with the Gaussian prior the subsequent Laplace approximation assumes.
    """
...
class ManualLastLayerLaplace:
    ...
    def fit(self, train_loader):
        ...
        # Reject shuffled loaders: the two-pass residual computation pairs
        # targets and predictions by order, which a shuffle would misalign.
