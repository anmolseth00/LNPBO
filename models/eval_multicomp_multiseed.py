...
def _build_features(df_split, cat_cols):
    fp_parts = []
    for comp in SMILES_COMPONENTS:
        col = SMILES_COL_MAP[comp]
        fp_parts.append(compute_morgan_fingerprints(df_split[col].tolist()))
    parts = fp_parts
    if cont_cols:
...
def _run_split(X_train, y_train, X_val, y_val, X_test, y_test, seed):
    ...
    print(f"    XGB: RMSE={xgb_m['rmse']:.4f}  R2={xgb_m['r2']:.4f}  ({xgb_time:.1f}s)")

    pred_ens = np.mean([pred_rf, pred_xgb], axis=0)
    ens_m = evaluate(y_test, pred_ens)
    split_results["rf_xgb"] = ens_m
    print(f"    R+X: RMSE={ens_m['rmse']:.4f}  R2={ens_m['r2']:.4f}")
...
for seed in SEEDS:
    ...
    X_train = _build_features(df_enc.iloc[train_idx], cat_cols)
    X_val = _build_features(df_enc.iloc[val_idx], cat_cols)
    X_test = _build_features(df_enc.iloc[test_idx], cat_cols)
    ...
    X_all = _build_features(df_enc_s, cat_cols_s)
...
# Summary
print(f"\n{'='*60}")
print("MULTI-COMPONENT SUMMARY (mean +/- std across 5 seeds)")
