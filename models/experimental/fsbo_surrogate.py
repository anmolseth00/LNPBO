...
def meta_train_gp(X_train, y_train, study_ids_train, train_study_ids,
    ...
    logger.info("  Meta-training on %d / %d points (from %d studies)",
                len(sub_idx), total_train, len(study_indices))
    ...
    logger.info("  Fitting meta-GP via L-BFGS-B ...")
    ...
    logger.info("  Meta-learned lengthscales: mean=%.3f, min=%.3f, max=%.3f",
                ls.mean(), ls.min(), ls.max())
    if "outputscale" in meta_params:
        logger.info("  Meta-learned outputscale: %.4f", meta_params['outputscale'].item())
    logger.info("  Meta-learned noise: %.6f", meta_params['noise'].item())
    logger.info("  Meta-learned mean: %.4f", meta_params['mean_constant'].item())
...
def _run_bo_loop(X, y, study_ids, test_study_ids, k_shots,
    ...
    for k in k_shots:
        ...
        if study_results:
            ...
            logger.info("  k=%d: top10=%.1f%% top50=%.1f%% (%d studies)",
                        k, results_by_k[k]['mean_top10_recall'] * 100,
                        results_by_k[k]['mean_top50_recall'] * 100,
                        len(study_results))
...
def main() -> int:
    ...
    logger.info("Train studies: %d, Test studies: %d", len(train_ids), len(test_ids))
    ...
    for k, r in random_results.items():
        logger.info("  k=%d: top10=%.1f%% top50=%.1f%%",
                     k, r['mean_top10_recall'] * 100, r['mean_top50_recall'] * 100)
    ...
    # Summary table
    logger.info("\n=== Summary ===")
    logger.info("%-20s %3s %8s %8s", "Method", "k", "Top-10", "Top-50")
    logger.info("-" * 45)
    for k in k_shots:
        methods = [
            ("Random", random_results),
        ...
        for name, res in methods:
            if k in res or str(k) in res:
                ...
                logger.info("%-20s %3d %7.1f%% %7.1f%%", name, k, t10 * 100, t50 * 100)
        logger.info("")
