#!/usr/bin/env python3
"""Train a D-MPNN model on LNPDB data for LNP property prediction.

Replaces the Chemprop v1.7 LiON model with a pure PyTorch implementation.
Trains multi-component D-MPNN (IL, HL, CHL, PEG) with tabular features
(molar ratios, mass ratio) and extracts learned molecular fingerprints.

Usage:
    python models/train_lion.py
    python models/train_lion.py --data-path /path/to/all_data_all.csv
    python models/train_lion.py --components IL --epochs 50 --batch-size 32
    python -m models.train_lion --help
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-path", type=str, default=None,
                        help="Path to all_data_all.csv (auto-detected if omitted)")
    parser.add_argument("--components", nargs="+", default=["IL"],
                        choices=["IL", "HL", "CHL", "PEG"],
                        help="Lipid components to encode with MPNN")
    parser.add_argument("--split", type=str, default="scaffold",
                        choices=["scaffold", "random"],
                        help="Train/val/test splitting strategy")
    ...
    parser.add_argument("--ffn-num-layers", type=int, default=2,
                        help="Number of hidden layers in the FFN (excl. output)")
    ...
    parser.add_argument("--no-categorical", action="store_true",
                        help="Disable one-hot categorical features (Model_type, Cargo, etc.)")
...
def evaluate(
    ...
    """Evaluate model, returning metrics in original (un-standardized) scale."""
    model.eval()
    all_preds = []
    all_targets = []
...
def extract_fingerprints(
    ...
    """Extract penultimate-layer fingerprints for all samples."""
    model.eval()
    fps = []
...
def save_plots(
    ...
    """Generate and save training curves and evaluation plots."""
    import matplotlib
    matplotlib.use("Agg")
...
def main():
    ...
    # Filter to rows with valid IL_SMILES (primary component) and target
    df = df.dropna(subset=["IL_SMILES", "Experiment_value"]).reset_index(drop=True)
    print(f"After filtering: {len(df)} rows with valid IL_SMILES and Experiment_value")

    # Determine continuous tabular columns that exist in the data
    tabular_cols = [c for c in TABULAR_CONTINUOUS_COLS if c in df.columns]
    print(f"Continuous features ({len(tabular_cols)}): {tabular_cols}")

    # -----------------------------------------------------------------------
    # Split (must happen before categorical encoding to avoid data leakage)
    # -----------------------------------------------------------------------
    print(f"Splitting data ({args.split})...")
    ...
    # One-hot encode categorical features (learn levels from training split only)
    ...
    if not args.no_categorical:
        ...
        print(f"Categorical features ({n_cat_cols} one-hot from "
              f"{len(cat_levels)} columns): {list(cat_levels.keys())}")
    else:
        print("Categorical features: disabled")
    ...
    # -----------------------------------------------------------------------
    print("Building datasets (featurizing molecules)...")
    t0 = time.time()
    ...
    print(f"Components: {args.components}, hidden={args.hidden_size}, "
          f"depth={args.depth}, ffn_layers={args.ffn_num_layers}, "
    ...
    scheduler = OneCycleLR(
        ...
        anneal_strategy="cos",
        final_div_factor=args.lr / 1e-5,  # final_lr ~= 1e-5
    )
    ...
    # -----------------------------------------------------------------------
    # Fingerprint extraction demo
    # -----------------------------------------------------------------------
    print("\nExtracting fingerprints from test set...")
    fps = extract_fingerprints(model, loader_test, device)
    print(f"Fingerprint shape: {fps.shape} (n_samples={fps.shape[0]}, dim={fps.shape[1]})")

    np.save(save_dir / "test_fingerprints.npy", fps)
    print(f"Saved to {save_dir / 'test_fingerprints.npy'}")
