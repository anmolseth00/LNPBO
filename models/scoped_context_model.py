#!/usr/bin/env python3
"""Scoped context-conditioned model: HeLa+FLuc in_vitro_single studies only.

Tests whether experimental context (Experiment_method, Experiment_batching)
...
def _study_split(df, seed=42):
    """80/20 study-level split (no stratification needed -- all same assay type)."""
    rng = np.random.RandomState(seed)
    study_ids = sorted(df["study_id"].unique().tolist())
...
def _within_study_r2(df, feat_cols, min_n=20, seed=42):
    ...
    for sid, sdf in df.groupby("study_id"):
        ...
        r2s.append(r2)
        study_r2s[str(sid)] = float(r2)

    return float(np.mean(r2s)) if r2s else float("nan"), r2s, study_r2s
...
def main() -> int:
    ...
    # Scope to HeLa + FLuc + in_vitro_single_formulation
    print("\nFiltering to HeLa + FLuc + in_vitro_single_formulation...")
    print(f"  Total rows before filter: {len(df)}")

    mask = (
        (df["assay_type"] == "in_vitro_single_formulation")
        & (df["Model_type"].str.lower() == "hela")
    )
    if "Cargo_type" in df.columns:
        cargo_mask = df["Cargo_type"].str.lower().isin(["fluc", "luc"])
    ...
    # Study summary
    study_counts = df.groupby("study_id").size().sort_values(ascending=False)
    print(f"  Studies: {len(study_counts)}")
    for sid, cnt in study_counts.items():
        print(f"    {sid}: {cnt} rows")

    # Context variation
    for col in ["Experiment_method", "Experiment_batching", "Model_type", "Cargo_type"]:
        if col in df.columns:
            vals = df[col].value_counts()
            print(f"  {col}: {dict(vals)}")

    # Encode LANTERN IL-only PCs
    print("\nEncoding LANTERN IL-only features...")
    ...
    for fs_name, feat_cols in feature_sets.items():
        ...
        for seed in seeds:
            ...
            ws_mean, ws_list, _ = _within_study_r2(train_df, feat_cols, min_n=20, seed=seed)
    ...
    # --- Within-study R² on full data ---
    print("\n" + "=" * 70)
    print("WITHIN-STUDY R² (per-study 80/20 formulation split, all studies)")
    print("=" * 70)

    for fs_name, feat_cols in feature_sets.items():
        print(f"\n--- {fs_name} ---")
        ws_mean, ws_list, ws_detail = _within_study_r2(encoded, feat_cols, min_n=20, seed=42)
        print(f"  Mean within-study R²: {ws_mean:.4f} ({len(ws_list)} studies)")
        for sid, r2 in sorted(ws_detail.items(), key=lambda x: x[1], reverse=True):
            n = int((encoded["study_id"] == sid).sum())
            print(f"    {sid} (n={n}): R²={r2:.4f}")
        results[f"{fs_name}_within_study_detail"] = ws_detail
    ...
    train_ids, _ = _study_split(encoded, seed=42)
    train_mask = encoded["study_id"].isin(train_ids)
    ...
    results["feature_importance"] = {
        ...
        "top_features": {k: float(v / total_imp) for k, v in feat_imp[:15]},
    }
