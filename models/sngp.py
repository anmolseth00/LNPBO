...
class SNGP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dims=(256, 128), n_random_features=1024,
        ...
        # Observation-noise variance (estimated from training residuals).
    ...
    def update_precision(self, x):
        ...
        with torch.no_grad():
            h = self.backbone(x)
            phi = self.rff(h)
            # Weight by likelihood precision 1/sigma_obs^2 (Liu et al. 2023).
    ...
    def predict_with_uncertainty(self, x):
        ...
        with torch.no_grad():
            ...
            # Total variance = aleatoric (observation noise) + epistemic (Laplace).
