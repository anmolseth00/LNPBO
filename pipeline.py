...
def main():
    ...
    parser.add_argument(
        ...
        help="Use a random subset of N rows from LNPDB (for fast iteration)"
    )
    parser.add_argument(
        "--batch-size", type=int, default=12,
        help="Number of formulations to suggest (default: 12)"
    )
    parser.add_argument(
        "--acq-type", type=str, default="UCB",
    ...
    parser.add_argument(
        "--il-pcs-morgan", type=int, default=5,
        help="Number of Morgan fingerprint PCs for ionizable lipid (default: 5)"
    )
    parser.add_argument(
        "--output", type=str, default="pipeline_output.csv",
        help="Output CSV path for suggested formulations"
    )
    parser.add_argument(
        "--study", type=str, default=None,
        help="Filter LNPDB to a specific publication PMID"
    )
    parser.add_argument(
        "--skip-bo", action="store_true",
        help="Skip BO step (just load data, encode, build space)"
    )
    parser.add_argument(
        "--reduction", type=str, default="pca",
        choices=["pca", "pls"],
    ...
    from LNPBO.data.lnpdb_bridge import load_lnpdb_full, list_available_datasets, lnpdb_csv_path
        ...
        if isinstance(val, dict) and "rows" in val:
            ...
            if key == "single_split":
                ...
                for hname, hinfo in val.items():
                    print(f"  heldout/{hname}: {hinfo.get('all_data_rows', '?')} rows, "
                          f"heldout={hinfo.get('heldout_data_rows', '?')} rows")
    ...
    print(f"\nIL:mRNA mass ratio: {il_mrna.nunique()} unique values "
          f"[{il_mrna.min():.1f}, {il_mrna.max():.1f}]")
    ...
    hl_n_pcs = 3 if (df["HL_name"].nunique() > 1 and "HL_SMILES" in df.columns and df["HL_SMILES"].nunique() > 1) else 0
    chl_n_pcs = 3 if (df["CHL_name"].nunique() > 1 and "CHL_SMILES" in df.columns and df["CHL_SMILES"].nunique() > 1) else 0
    peg_n_pcs = 3 if (df["PEG_name"].nunique() > 1 and "PEG_SMILES" in df.columns and df["PEG_SMILES"].nunique() > 1) else 0
    ...
    print(f"  Molar ratio bounds:")
    for role, bounds in space.molratio_bounds.items():
        print(f"    {role}: [{bounds[0]:.4f}, {bounds[1]:.4f}]")
    print(f"  IL:mRNA values: {space.il_mrna_massratio_values[:5]}{'...' if len(space.il_mrna_massratio_values) > 5 else ''}")
