...
def chemeleon_embeddings(
    ...
    def _compute(todo):
        ...
        # Failed SMILES become zero vectors. Always warn; raise above 5%.
        if n_failed > 0:
            pct = 100.0 * n_failed / len(todo) if todo else 0.0
            logger.warning(
                "CheMeleon: %d/%d SMILES (%.1f%%) failed to parse - zero vectors used",
                n_failed, len(todo), pct,
            )
            if pct > 5.0:
                raise ValueError(
                    f"CheMeleon: {pct:.1f}% of SMILES ({n_failed}/{len(todo)}) failed to "
                    "encode and would become zero vectors (threshold 5%). Aborting."
                )
        return result
