...
def add_study_id(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Publication_PMID" in df.columns:
        base = df["Publication_PMID"].astype("string").str.replace(r"\.0$", "", regex=True)
        if "Experiment_ID" in df.columns:
            base = base.fillna(df["Experiment_ID"].astype("string"))
        if "Publication_link" in df.columns:
            base = base.fillna(df["Publication_link"].astype("string"))
        # For multi-Model_type PMIDs, create composite study_id
        # (z-scores are computed per (Experiment_ID, Model_type))
        if "Model_type" in df.columns:
            multi_mt = df.groupby("Publication_PMID")["Model_type"].nunique()
            multi_mt_pmids = set(multi_mt[multi_mt > 1].index)
            mt = df["Model_type"].astype("string")
            is_multi = df["Publication_PMID"].isin(multi_mt_pmids)
            study = base.copy()
            study[is_multi] = base[is_multi] + "_" + mt[is_multi]
            df["study_id"] = study
        else:
            df["study_id"] = base
...
def study_split(df_or_ids, study_to_type: dict[str, str] | None = None, *, seed: int = 42) -> tuple[set, set]:
