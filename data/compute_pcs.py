    ...
    if fitted_reducer is not None:
        ...
        if reduction == "none":
            ...
            try:
                ...
                # Silently falling back to PCA would mislabel the result as PLS,
                # so fail loudly instead and let the caller decide (e.g. switch
                # to reduction='pca' explicitly, or drop the role).
                raise ValueError(
                    f"PLS reduction failed for feature_type={feature_type!r} with "
                    f"n_components={n_components} ({fp_scaled.shape[0]} samples, "
                    f"{fp_scaled.shape[1]} features): {e}. Refusing to silently fall "
                    "back to PCA (would mislabel results as PLS). Use reduction='pca' "
                    "explicitly if a PCA basis is acceptable."
                ) from e
        else:
