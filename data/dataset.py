...
LNPDB_REQUIRED_COLUMNS = [
    "IL_name", "IL_to_nucleicacid_massratio", "IL_molratio",
    "HL_name", "HL_molratio",
    "CHL_name", "CHL_molratio",
    "PEG_name", "PEG_molratio",
    "Experiment_value", 
]

INGREDIENT_COLUMNS = [
    "IL_name", "IL_SMILES", "IL_to_nucleicacid_massratio", "IL_molratio",
    "IL_to_nucleicacid_upper", "IL_to_nucleicacid_lower", "IL_upper", "IL_lower",
    "HL_name", "HL_SMILES", "HL_molratio", "HL_upper", "HL_lower",
    "CHL_name", "CHL_SMILES", "CHL_molratio", "CHL_upper", "CHL_lower",
    "PEG_name", "PEG_SMILES", "PEG_molratio", "PEG_upper", "PEG_lower",
]
...
class Dataset:
    ...
    def from_lnpdb_csv(cls, path: str) -> "Dataset":
        ...
        if incomplete_mask.any():
            ...
            raise ValueError(
                f"Incomplete rows detected at indices {bad_rows}. "
    ...
    def encode_dataset(
        ...
        def encode_lipid_table(lipid_df: pd.DataFrame, role: str, pcs_spec: dict):
            ...
            for enc_type, n in pcs_spec.items():
                ...
                cols = [
                    ...
                    for i in range(pc_list.shape[1])
                ]
            ...
            return pd.concat(
                [lipid_df.reset_index(drop=True)] + blocks,
                axis=1
            )
        ...
        if encoding_csv_path:
            ...
            if only_encodings:
                if encoding_tables:
                    pd.concat(encoding_tables, axis=0).to_csv(
                        encoding_csv_path, index=False
                    )
            else:
                ordered_base_cols = [
                    "IL_name", "IL_SMILES", "IL_molratio", "IL_to_nucleicacid_massratio",
    ...
    def from_doe_options(cls, path: str) -> Dict[str, Dict]:
        ...
        for _, row in df.iterrows():
            ...
            options[role] = {
                ...
                "ratios": (
                    [float(x) for x in row["ratios"].split(";")]
                    if pd.notna(row["ratios"])
                    else None
                ),
                "bounds": (
                    (row["ratio_lower"], row["ratio_upper"])
                    if pd.notna(row["ratio_lower"])
                    else None
                ),
            }
        return options
    ...
    def _ensure_df(self, x, prefix):
        ...
        return pd.DataFrame(
            x,
            columns=[f"{prefix}pc{i+1}" for i in range(x.shape[1])]
        )
    ...
    def append_suggestions(self, df_new: pd.DataFrame) -> "Dataset":
        ...
        feature_cols = [
            c for c in columns_to_check_for_duplicates
            if c in df_new.columns and c in self.df.columns
        ]

        combined_check = pd.concat(
            [self.df[feature_cols], df_new[feature_cols]],
            ignore_index=True
        )

        if combined_check.duplicated().any():
            raise ValueError(
                "Duplicate formulations detected when appending suggestions."
            )
