...
class Dataset:
    ...
    def encode_dataset(
        ...
        *,
        ...
        # Data-leakage guard for SUPERVISED reduction. A fresh PLS fit (no
        # pre-fitted transformers) consumes per-lipid Experiment_value targets;
        # fitting it over a frame that still contains held-out rows leaks those
        # targets into the encoder basis. Require the caller to either (a) pass
        # ``target_train_idx`` asserting the frame is pre-split / train-only, or
        # (b) explicitly opt in via ``allow_fit_on_all=True``. Transform-only
        # calls (``fitted_transformers_in``) are exempt; unsupervised PCA / no
        # reduction are not target-leaky and are left unguarded so single-frame
        # deployment encoding keeps working.
        if (
            reduction == "pls"
            and fitted_transformers_in is None
            and target_train_idx is None
            and not allow_fit_on_all
        ):
            raise ValueError(
                "encode_dataset(reduction='pls') is about to fit PLS on ALL rows, leaking "
                "held-out targets into the encoder basis. Pass target_train_idx (the "
                "training row labels; use list(dataset.df.index) if the frame is already "
                "train-only), supply fitted_transformers_in for transform-only encoding, "
                "or pass allow_fit_on_all=True to explicitly accept fitting on the full frame."
            )
        ...
        def average_experiment_values(role: str) -> dict[str, float]:
            ...
            rows) leaks held-out targets into the encoder. ``target_train_idx``
            (the training row labels) is therefore required: the caller must
            pre-split the frame and pass the training index explicitly. If the
            frame is already train-only, pass its full index
            (e.g. ``list(self.df.index)``).
            """
            if target_train_idx is None:
                raise ValueError(
                    "average_experiment_values() requires an explicit train index to avoid "
                    "leaking held-out targets into PLS/LiON encoder fitting. Pass "
                    "encode_dataset(..., target_train_idx=<training row labels>). If the frame "
                    "is already train-only, pass its full index (list(dataset.df.index))."
                )
            name_col = f"{role}_name"
            src = df.loc[df.index.intersection(target_train_idx)]
        ...
        def encode_lipid_table(lipid_df: pd.DataFrame, role: str, pcs_spec: dict):
            ...
            for enc_type, n in pcs_spec.items():
                ...
                from .compute_pcs import compute_pcs  # deferred: keeps Dataset import light
    ...
    def refit_pls(self, training_indices: list[int], external_df=None) -> None:
        ...
        for key, raw_data in self.raw_fingerprints.items():
            ...
            if train_mask.sum() < 2:
                # Cannot refit PLS for this role; existing PCs are kept.
                # Make this loud and tag the role's provenance so a stale-PLS
                # reduction is never silently presented as a fresh PLS fit.
                logger.warning(
                    "refit_pls: role %r (%s) has only %d labeled training lipids (<2); "
                    "keeping prior PLS PCs (not refit on the current training split).",
                    role, enc_type, int(train_mask.sum()),
                )
                if key in self.fitted_transformers:
                    self.fitted_transformers[key]["reduction_method"] = "pls_stale_not_refit"
                continue
            ...
            try:
                ...
                # PLS can fail with degenerate features (e.g., 3 unique PEG
                # SMILES). Raising here would break legitimate low-cardinality
                # helper roles in the headline PLS path, so we keep the prior
                # PLS PCs but make it loud and tag the role's provenance so the
                # stale reduction can never be mislabeled as a fresh PLS fit.
                logger.warning(
                    "refit_pls: PLS refit failed for role %r (%s), %d train samples / "
                    "%d features: %s. Keeping prior PLS PCs (stale, not refit).",
                    role, enc_type, fp_train.shape[0], fp_train.shape[1], e,
                )
                if key in self.fitted_transformers:
                    self.fitted_transformers[key]["reduction_method"] = "pls_stale_not_refit"
                continue

            # Update fitted_transformers (successful refit)
            if key in self.fitted_transformers:
                self.fitted_transformers[key]["reducer"] = reducer
                self.fitted_transformers[key]["reduction_method"] = "pls"
