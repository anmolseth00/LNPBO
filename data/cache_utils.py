...
def cached_encode(
    ...
    # Unknown SMILES fall back to zero vectors, which silently degrade
    # downstream models. Count failures, always warn, and raise above 5%.
    n_missing = sum(1 for s in list_of_smiles if s not in cache)
    if n_missing > 0:
        n_total = len(list_of_smiles)
        pct = 100.0 * n_missing / n_total if n_total else 0.0
        logger.warning(
            "%s: %d/%d SMILES (%.1f%%) not in cache - replaced with zero vectors",
            label, n_missing, n_total, pct,
        )
        if pct > 5.0:
            raise ValueError(
                f"{label}: {pct:.1f}% of SMILES ({n_missing}/{n_total}) could not be "
                "encoded and would become zero vectors (threshold 5%). Aborting to avoid "
