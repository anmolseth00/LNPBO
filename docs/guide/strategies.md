...
The scikit-learn GP backend, using `GaussianProcessRegressor`. Uses continuous acquisition optimization with L-BFGS-B, requiring continuous relaxation of discrete variables.
...
NGBoost is a strong default surrogate for fixed-ratio library screening.
...
CASMOPolitan is particularly strong on ratio-only optimization tasks, where it can leverage the inherent mixed structure of the search space.
