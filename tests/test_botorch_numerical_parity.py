...
def _setup(n_train=80, n_pool=200, d=5, seed=42):
    ...
    y_train = np.sin(X_train[:, 0]) + X_train[:, 1] ** 2 + 0.1 * rng.randn(n_train)
...
class TestUCBParity:
    ...
    def test_ucb_exact_match(self):
        ...
        model, _, _y_train, X_pool, y_best = _setup()
        kappa = 2.576
    ...
    def test_ucb_ranking_identical(self):
        ...
        model, _, _y_train, X_pool, y_best = _setup(seed=123)
        kappa = 2.0
        device = next(model.parameters()).device
...
class TestEIParity:
    ...
    def test_ei_ranking_top10(self):
        ...
        model, _, _y_train, X_pool, y_best = _setup(seed=99)
        xi = 0.01
        device = next(model.parameters()).device
        ...
        assert overlap >= 8, f"EI top-10 overlap: {overlap}/10 — rankings should mostly agree"
...
class TestImplementationDifferences:
    ...
    def test_std_clamp_difference(self):
        ...
        model, _, _y_train, X_pool, _y_best = _setup()

        _mean, std = predict(model, X_pool)
        ...
        assert n_clamped == 0, f"{n_clamped} pool points have std < 1e-10; clamp difference only matters when std → 0"
