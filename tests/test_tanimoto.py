...
class TestTanimotoKernel:
    ...
    def test_binary_matches_jaccard(self):
        ...
        k_val = kernel(x, y).evaluate().item()
        assert abs(k_val - expected) < 1e-6, f"Expected Jaccard={expected:.4f}, got Tanimoto={k_val:.4f}"
    ...
    def test_diag_mode(self):
        ...
        torch.testing.assert_close(
            diag,
            torch.ones(15, dtype=torch.float64),
            atol=1e-6,
            rtol=0,
        )
    ...
    def test_count_vectors(self):
        ...
        expected = 4.0 / 16.0
        k_val = kernel(x, y).evaluate().item()
        assert abs(k_val - expected) < 1e-6, f"Expected {expected:.4f} for count vectors, got {k_val:.4f}"
...
class TestTanimotoGP:
    ...
    def test_tanimoto_acquisition_scores_finite(self):
        ...
        for acq_type in ["UCB", "EI", "LogEI"]:
            ...
            assert np.all(np.isfinite(scores)), f"{acq_type} scores contain NaN/inf with Tanimoto kernel"
    ...
    def test_tanimoto_bo_loop(self):
        ...
        for round_i in range(n_rounds):
            ...
            assert len(batch) == batch_size, f"Round {round_i}: expected {batch_size}, got {len(batch)}"
            for idx in batch:
