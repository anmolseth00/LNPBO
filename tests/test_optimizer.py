...
class TestNormalization:
    ...
    def test_zscore_constant_returns_centered(self):
...
class TestSuggestDispatch:
    ...
    def test_gp_mixed_dispatches_to_gp_mixed(self):
        assert SURROGATE_TYPES["gp_mixed"] == "gp_mixed"
...
class TestGPEngine:
    def test_default_engine_is_botorch(self):
        o = _make_optimizer(surrogate_type="gp")
        assert o._family == "gp_botorch"

    def test_sklearn_engine_resolves(self):
        o = _make_optimizer(surrogate_type="gp", gp_engine="sklearn")
        assert o._family == "gp_sklearn"

    def test_gp_sklearn_alias_normalizes(self):
        o = Optimizer.__new__(Optimizer)
        o.surrogate_type = "gp_sklearn"
        o.gp_engine = "botorch"
        o.acquisition_type = "UCB"
        o.batch_strategy = "kb"
        o.normalize = "copula"
        # Simulate __init__ normalization
        if o.surrogate_type == "gp_sklearn":
            o.surrogate_type = "gp"
            o.gp_engine = "sklearn"
        assert o.surrogate_type == "gp"
        assert o.gp_engine == "sklearn"
        assert o._family == "gp_sklearn"

    def test_invalid_engine_rejected(self):
        o = _make_optimizer(surrogate_type="gp", gp_engine="invalid")
        with pytest.raises(ValueError, match="Unknown gp_engine"):
            o._validate_config()

    def test_engine_ignored_for_discrete(self):
        o = _make_optimizer(surrogate_type="xgb_ucb", gp_engine="sklearn")
        assert o._family == "discrete"


# ---------------------------------------------------------------------------
# 7. Kernel-feature compatibility warnings
# ---------------------------------------------------------------------------


class TestKernelFeatureWarnings:
    def test_tanimoto_pca_warning(self):
        """Tanimoto kernel with PCA-reduced fingerprints should warn."""
        pool = _make_synthetic_df()
        # _make_synthetic_df already has IL_count_mfp_pc* columns
        o = Optimizer.__new__(Optimizer)
        o.surrogate_type = "gp"
        o.gp_engine = "botorch"
        o.acquisition_type = "UCB"
        o.batch_strategy = "kb"
        o.normalize = "copula"
        o.kernel_type = "tanimoto"
        o.kernel_kwargs = None
        o.candidate_pool = pool
        with pytest.warns(UserWarning, match="tanimoto.*PCA-reduced"):
            o._validate_config()
