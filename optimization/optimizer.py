...
**Preference / domain-robust** (discrete pool scoring):
    ...
    ``"bradley_terry"`` Pairwise preference model - learns utility from
                        pairwise comparisons (Bradley & Terry 1952).
...
# Compound acquisition type mapping for the sklearn GP pipeline
_SKLEARN_ACQ_MAP = {
    ("UCB", "kb"): "UCB",
    ("EI", "kb"): "EI",
    ("LogEI", "kb"): "LogEI",
...
class Optimizer:
    ...
    def has_unrepresented_runtime_state(self) -> bool:
        """Whether this optimizer maintains cross-round state not captured by a
        round-level checkpoint.

        Resuming such an optimizer from a partial checkpoint would silently
        drop that state (e.g. casmopolitan's adaptive trust region), so callers
        should refuse to resume from a checkpoint when this is True.
        """
    ...
    def suggest_indices(
        ...
        Low-level batch-selection API. Operates on a
        ...
        X_train, y_train, X_pool, pool_arr, valid_train, valid_pool, raw_y_train = self._prepare_indices_data(
        ...
        # For reproducibility, force single-threaded surrogates
    ...
    def _prepare_indices_data(self, df, feature_cols, training_idx, pool_idx):
        ...
        raw_y_train = df.loc[training_idx, "Experiment_value"].values.astype(np.float64)
        y_train = normalize_values(raw_y_train, self.normalize)
        ...
        raw_y_train = raw_y_train[valid_train]
        ...
        return X_train, y_train, X_pool, pool_arr, valid_train, valid_pool, raw_y_train
    ...
    def _run_batch_selection(
        ...
        if family == "casmopolitan":
            ...
            # Build IL categorical encoding. Sort the unique IL names so each
            # name maps to a stable integer code across rounds: train + pool is
            # the same set every round (batches only move rows between them), so
            # sorting keeps the codes stored in the restart archive valid. Plain
            # .unique() uses first-appearance order, which drifts as rows move
            # from pool to train and would corrupt restart-center selection.
            il_train, il_pool = il_names
            all_il = pd.concat([il_train, il_pool], ignore_index=True)
            il_map = {name: i for i, name in enumerate(sorted(all_il.unique()))}
            ...
            # Trust-region success/failure must be judged on RAW Experiment_value.
            # The normalized y_train is refit each round (copula ranks shift,
            # z-score mean/std shift), so np.max(y_train) grows with N even when
            # no new best is found, making `improved` always True and disabling
            # TR shrink/restart. raw_y_train is monotonic in y_train, so argmax
            # (the incumbent) is unchanged.
            y_best = raw_y_train if raw_y_train is not None else y_train
            current_best = float(np.max(y_best))
            ...
            if self._casmopolitan_trust_region is not None and self._casmopolitan_round_start_best is not None:
                ...
                if restart_from_archive:
                    incumbent_idx = int(np.argmax(y_best))
    ...
    def _suggest_pool_based(self, output_csv):
        ...
        raw_y_train = train_df["Experiment_value"].values.astype(np.float64)
    ...
    def _suggest_gp_sklearn(self, output_csv):
        """Suggest a batch using the sklearn GP pipeline.
        ...
        def decode_component(df_new, role, space):
            ...
            from sklearn.metrics import pairwise_distances  # deferred: heavy, decode-path only
