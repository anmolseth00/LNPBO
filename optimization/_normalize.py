"""Target normalization utilities for the Optimizer."""
...
def copula_transform(values, x_new=None):
    ...
    ref = np.sort(values)
    n = len(ref)
    # Use the same Hazen plotting position (rank - 0.5)/n as the fit branch
    # above, so a value maps to the same normal score whether it came from the
    # reference set or is passed as x_new. The previous (rank + 0.5)/(n + 1)
    # put x_new on a slightly different scale than the trained targets, biasing
    # any coverage check that compares the two.
    ranks = np.searchsorted(ref, x_new, side="right").astype(float)
    u = np.clip((ranks - 0.5) / n, 1e-10, 1 - 1e-10)
    return _norm.ppf(u)
