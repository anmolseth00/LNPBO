...
def optimize_mixed_acquisition(
    ...
    # Restrict the categorical candidate set to the trust region in BOTH
    # branches. CASMOPOLITAN optimizes acquisition only over categories within
    # the categorical TR (those differing from the incumbent in at most
    # n_cat_perturb dims). Previously the small-cat-count branch skipped this
    # filter entirely, making the categorical TR a silent no-op for studies
    # with <= n_cat_samples unique categoricals (essentially all studies).
    ...
    if unique_cats.shape[0] <= n_cat_samples:
        # Small candidate set: use all in-TR categories. Fall back to the
        # incumbent's category if the TR would empty the pool (avoids an
        # empty candidate set when no enumerated category is inside the TR).
        if len(in_tr_cats) > 0:
            cat_candidates = in_tr_cats
        else:
            cat_candidates = trust_region.center_cat.reshape(1, -1)
    else:
